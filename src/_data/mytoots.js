...
const fs = require('fs');
const unionBy = require('lodash/unionBy');
const {removeMastoTags} = require("../_11ty/helpers");
...
module.exports = async function () {
    const options = {
        isProduction: process.env.ELEVENTY_ENV !== "development",
        host: 'https://toot.cat',
        userId: '109260262121658226',
        removeSyndicates: ['geekosaur.com'],
        cacheLocation: "src/_cache/mastonotes.json",
        removeTags: true,
    ...
    if (config.isProduction) {
        ...
        if (feed) {
            ...
            return compute(mastodonPosts.posts);
        }
    }

    return compute(cache.posts);
	// });
};