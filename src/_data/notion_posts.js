...
async function fetchPage(pageId) {
  if (process.env.ELEVENTY_ENV === "devbuild") {
    ...
    n2m.setCustomTransformer("image", async (block) => {
      ...
      if (image_type === "file") {
        const localImage = await downloadNotionImage(
          pageId,
...
async function fetchPosts(since) {
  ...
  if (results) {
    ...
    for (const post of results) {
      ...
      props["thumbnail"] = await downloadNotionImage(
        post.id,
        props["thumbnail"][0]
      );
...
module.exports = async function () {
  ...
  if (cache.data && Object.keys(cache.data).length) {
    console.log(
      `>>> ${Object.keys(cache.data).length} posts loaded from cache`
    );
  }
  ...
  await updateToot(notion, newData, "post");
